ecloir: CyanogenMod 5.0.6 Restoration Project 
===========================================

This branch contains a reconstructed `repo` local manifest of the CyanogenMod 5.0.6 ("Eclair") build to be available via CyanogenMod source code.
...
It is recommended to use an older Linux distribution. All builds will be tested on Ubuntu 12.04 LTS ("Precise Pangolin"), which can be downloaded from [here](https://old-releases.ubuntu.com/releases/12.04/ubuntu-12.04.5-desktop-amd64.iso). 
...
Java 5 is required for compiling CM 5.0.6. To install it, run the script again - select the 2nd option in the main menu and then select option 1. The script can also change the default Java version, which can be useful if compiling different CyanogenMod versions.
...
To initialize a repository tree using one of the manifests provided by this project, execute a command like this (see the list of manifests above for available `<build>`s):
    ...
    repo init -u https://github.com/jellyclair/manifest -b cm-5.0.6

Then add the CyanogenMod 5.0.6 local manifest to the folder:
...
The CyanogenMod 5.0.6 build will then likely fail half way through on 12.04. Re-type the script again, although replace 4.2 with 4.4.
...
Credits
-------
Froyocomb (for most of the parts)
